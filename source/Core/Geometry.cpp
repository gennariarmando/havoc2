...
void CGeometry::Render() {
    ...
    if (!m_pShader)
        m_pShader = DeviceObjects.m_pDefaultShader;
    ...
    // fs
    m_pShader->SetShaderUniform("projection", Camera.GetProjection());
    m_pShader->SetShaderUniform("view", Camera.GetView());
...
void CGeometry::SetShader(std::shared_ptr<CShader> s) {
    m_pShader = s;
}
