...
void CCamera::ProcessDebug() {
    ...
    right = DeviceObjects.m_vMouseDelta.x * 0.05f;
    up = DeviceObjects.m_vMouseDelta.y * 0.05f;
    ...
    float velocity = 10.0f * static_cast<float>(DeviceObjects.m_fDeltaTime);
    float forward = 0.0f;
    float side = 0.0f;

    if (BaseDevice->GetKeyDown(GLFW_KEY_LEFT_SHIFT))
        velocity = 50.0f * static_cast<float>(DeviceObjects.m_fDeltaTime);

    if (BaseDevice->GetKeyDown(GLFW_KEY_W))
        forward = velocity;
    if (BaseDevice->GetKeyDown(GLFW_KEY_S))
        forward = -velocity;

    if (BaseDevice->GetKeyDown(GLFW_KEY_A))
        side = -velocity;
    if (BaseDevice->GetKeyDown(GLFW_KEY_D))
        side = velocity;

    if (BaseDevice->GetKeyDown(GLFW_KEY_R))
        up = velocity;
    if (BaseDevice->GetKeyDown(GLFW_KEY_F))
        up = -velocity;
